...
class kh:
    def __init__(self, i, name, room, d1, d2, plus) -> None:
        ...
        self.name = name
        self.room = room
        self.days = (datetime.strptime(d2, "%d/%m/%Y") - datetime.strptime(d1, "%d/%m/%Y")).days + 1
        self.plus = plus
...
for i in range(int(input())):
    a.append(kh(i+1, input(), input(), input().strip(), input().strip(), int(input())))
for i in sorted(a, key=lambda x: -x.sum):
    print(i)